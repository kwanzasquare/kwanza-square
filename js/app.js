...
(function (KZ) {
  ...
  // ------------------------------------------------------------- halftime
  //
  // Martin's ruling, in his own words: "You do not save the game to reopen.
  // You only save the result." A match is a football match — two halves, the
  // score kept between them, halftime as long as the player wants, extra time
  // only when it is 1-1. A finished round is finished.
  //
  // So this stores the SCORE and the history, never a position. Every round
  // begins from placement on an empty board, round 2 exactly like round 1,
  // which is why there is nothing here a player could reload to replay a round
  // that went badly. The hole closes by construction rather than by policing.
  //
  // One thing must travel with the score: the action log. The leaderboard
  // believes nothing the client says about the outcome and replays the whole
  // match on the server, so the moves already played have to survive the break
  // or a finished match could never be submitted. A log is history, not a
  // position — it cannot be loaded back onto a board — so it reopens nothing.
  //
  // THE SLOT IS USED ONCE. Written when a round ends, deleted the moment the
  // next round begins. Walk away in the middle of a half and the match is gone,
  // the same as walking off the pitch — which is also what stops "close the tab
  // while losing and try that round again".

  var HALFTIME = 'kwanza-halftime';
  var HALFTIME_V = 2;

  /**
   * A fingerprint of the rules this save was played under.
   *
   * A paused match is only submittable while the server can still replay it
   * under the same rules. If the soldier count or any rule ever changes, an old
   * save becomes unverifiable — so it is discarded on load, here, rather than
   * failing mysteriously at submit time weeks later in somebody else's hands.
   */
  function rulesSignature(m) {
    return [HALFTIME_V, E.PAWNS_PER_SIDE, E.ROUNDS_TO_WIN, E.DRAW_LIMIT,
            E.MIN_SOLDIERS, G.NODE_COUNT, G.trios.length,
            m.pawnsPerSide, m.roundsToWin, m.difficulty, m.mode].join(':');
  }

  function saveHalftime() {
    if (!state || state.matchOver) return;
    try {
      localStorage.setItem(HALFTIME, JSON.stringify({
        v: HALFTIME_V,
        rules: rulesSignature(state),
        at: Date.now(),
        round: state.round,
        scores: state.scores,
        ...
        opponent: settings.opponent, arena: settings.arena,
        cards: cards
      }));
    } catch (e) { /* private mode — the match simply cannot be paused */ }
  }

  function loadHalftime() {
    try {
      var raw = localStorage.getItem(HALFTIME);
      if (!raw) return null;
      var m = JSON.parse(raw);
      if (!m || m.v !== HALFTIME_V || m.rules !== rulesSignature(m)) {
        clearHalftime();
        return null;
      }
      return m;
  ...
  function clearHalftime() {
    try { localStorage.removeItem(HALFTIME); } catch (e) {}
  }

  /**
   * Football words, which Martin asked for and which do real work: nobody needs
   * "half time" explained, and everybody already knows you do not replay the
   * first half because you disliked the score.
   */
  function halftimeWords(scores, round, roundsToWin) {
    if (round === 1) {
      return { title: 'Half time', go: 'Play the second half',
               pause: 'Take halftime', next: 'The second half' };
    }
    if (round === 2 && roundsToWin === 2 && scores.A === scores.B) {
      return { title: 'Full time — level', go: 'Play extra time',
               pause: 'Take a break', next: 'Extra time' };
    }
    return { title: 'End of round ' + round, go: 'Play round ' + (round + 1),
             pause: 'Come back later', next: 'Round ' + (round + 1) };
  }

  /** Pick up a paused match. The slot is spent the instant this runs. */
  function resumeMatch(m) {
    state = E.createMatch({
      mode: m.mode, aiSide: m.aiSide, difficulty: m.difficulty,
      pawns: m.pawnsPerSide, roundsToWin: m.roundsToWin
    });
    if (m.names) state.names = m.names;
    state.scores = { A: m.scores.A, B: m.scores.B };
    state.roundHistory = m.roundHistory || [];
    state.actionLog = m.actionLog || [];
    state.round = m.round;
    state.startingPlayer = m.startingPlayer;
    // nextRound() advances the round, alternates who opens, clears the board and
    // stamps 'R' into the log — the identical boundary the live game crosses, so
    // a resumed match replays on the server exactly like an unbroken one.
    E.nextRound(state);

    cards = m.cards || {};
    ['A', 'B'].forEach(function (side) {
      if (cards[side] && !cards[side].moves) cards[side].moves = [];
    });
    clearHalftime();

    if (m.opponent && R.SOLDIERS[m.opponent]) settings.opponent = m.opponent;
    if (m.arena) settings.arena = m.arena;
    saveSettings();

    history = []; selected = null; staged = null; thinking = false;
  ...
  /** The card on the home screen offering the paused match back. */
  function renderResume() {
    var box = $('#home-resume');
    if (!box) return;
    var m = loadHalftime();
    if (!m) { box.hidden = true; box.innerHTML = ''; return; }
    var w = halftimeWords(m.scores, m.round, m.roundsToWin);
    var me = m.mode === 'ai' ? (m.aiSide === 'A' ? 'B' : 'A') : 'A';
    var foe = me === 'A' ? 'B' : 'A';
    box.hidden = false;
    box.innerHTML =
      '<div class="resume-head">' +
        '<span class="resume-title">' + w.title + '</span>' +
        '<span class="resume-score">' + m.scores[me] + ' – ' + m.scores[foe] + '</span>' +
      '</div>' +
      '<p class="resume-note">' + w.next + ' starts on a fresh board, and the score is waiting.</p>' +
      '<div class="resume-actions">' +
        '<button class="btn btn-sm btn-gold" id="btn-resume">' + w.go + '</button>' +
        '<button class="btn btn-sm btn-ghost" id="btn-forfeit">Give it up</button>' +
      '</div>';
  }
  ...
  function showScreen(id) {
    ...
    if (id === 'screen-home') { loadHomeBoard(); renderResume(); }
  }
  ...
  function showRoundResult() {
    ...
    // The round is over and the score is now history, so write the slot before
    // anything else: closing the tab at this exact moment must be safe.
    saveHalftime();
    var words = halftimeWords(state.scores, state.round, state.roundsToWin);

    function playOn() {
      closeModal();
      KZ.Celebrate.stop();
      clearHalftime();          // the slot is spent the moment play resumes
      E.nextRound(state);
      R.clearPrisoners(view);
      history = [];
    ...
    openModal(
      '<div class="result-head">' +
        ...
        grade +
        '<div class="result-break"><b>' + words.title + '.</b> The score is kept. ' +
          'Come back in five minutes or five days — ' + words.next.toLowerCase() +
          ' starts on a fresh board.</div>' +
      '</div>',
      [
        { label: words.go, cls: playerWon ? 'btn-gold' : 'btn-primary', onClick: playOn },
        { label: words.pause, onClick: function () {
            closeModal();
            KZ.Celebrate.stop();
  ...
  function startMatch() {
    // One slot per match: a new match ends whatever was paused.
    clearHalftime();
    state = E.createMatch({
      mode: settings.mode,
      aiSide: 'B',
  ...
  function init() {
    ...
    $('#home-resume').addEventListener('click', function (ev) {
      if (ev.target.closest('#btn-resume')) {
        var m = loadHalftime();
        if (m) resumeMatch(m);
        else renderResume();
      } else if (ev.target.closest('#btn-forfeit')) {
        openModal('<h2>Give up the match?</h2><div class="modal-body"><p>The score is lost ' +
          'and nothing goes to the leaderboard.</p></div>', [
          { label: 'Keep it', onClick: closeModal },
          { label: 'Give it up', cls: 'btn-danger', onClick: function () {
              closeModal(); clearHalftime(); renderResume();
            } }
        ]);
      }
    });
    ...
    $('#btn-start').addEventListener('click', function () {
      var paused = loadHalftime();
      if (!paused) { startMatch(); return; }
      var w = halftimeWords(paused.scores, paused.round, paused.roundsToWin);
      openModal('<h2>A match is waiting</h2><div class="modal-body"><p>' + w.title +
        ' at ' + paused.scores.A + '–' + paused.scores.B +
        '. Starting a new match ends it, and nothing is submitted.</p></div>', [
        { label: 'Keep it', onClick: function () { closeModal(); showScreen('screen-home'); } },
        { label: 'Start a new match', cls: 'btn-danger', onClick: function () { closeModal(); startMatch(); } }
      ]);
    });
